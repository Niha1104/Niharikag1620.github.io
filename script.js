...
(function revealSetup() {
  const targets = document.querySelectorAll(
    '.section__head, .about__text, .about__stats, .timeline__item, .featured-project, .project-card, .skill-group, .contact__row'
  );
...
(function heroGraph() {
  ...
  function computeEdges(projected) {
    const list = [];
  ...
  function resize() {
    w = canvas.parentElement ? window.innerWidth : window.innerWidth;
  ...
  const accentColor = () => {
    ...
    return {
      accent: style.getPropertyValue('--accent').trim() || '#38E1C6',
      accentRgb: style.getPropertyValue('--accent-rgb').trim() || '56,225,198',
      accent2Rgb: style.getPropertyValue('--accent-2-rgb').trim() || '245,166,91',
    };
  };
  ...
  function draw() {
    ...
    const { accentRgb, accent2Rgb } = accentColor();

    const projected = nodes.map(n => project(n, angle, mouseX, mouseY));

    // recompute edges occasionally (every ~40 frames) for perf, based on raw 3D positions
    draw._frame = (draw._frame || 0) + 1;
    if (draw._frame % 40 === 1) {
      edges = computeEdges();
    ...
    edges.forEach(([i, j, dist]) => {
      ...
      const opacity = Math.max(0, (1 - dist / CONNECT_DIST) * 0.35 * avgScale);
    ...
    // draw nodes
    projected.forEach(p => {
      const r = Math.max(1, 2.4 * p.scale);
    ...
    if (!reduced) {
      particles.forEach((p, idx) => {
        ...
        const avgScale = (a.scale + b.scale) / 2;
        const r = Math.max(1.2, 2.6 * avgScale);
